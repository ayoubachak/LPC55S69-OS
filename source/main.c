...
int main()
{
	if(task_new(code1, 512) < 0)
		printf("Cannot start task 1");// tache 1
	if(task_new(code2, 512) < 0)   // tache 2
		printf("Cannot start task 1");// tache 1
	if(task_new(code3, 512) < 0)   // tache 3
		printf("Cannot start task 1");// tache 1
...
void tache1()
{
    int serial=open("/dev/serial",O_READ | O_WRITE);
    char buff[255];

    char* strDebug = "Ceci est un message";
    write(serial,strDebug,sizeof(strDebug));

    while (1)
    {
        read(serial,(void*)buff,1); //read 1 char
