...
static int dev_init_btn(Device *dev)
{
    ...
    dev->sem_read = sem_new(1);//init read sem
...
static int dev_read_btn(FileObject *f, void *buf, size_t len)
{
	sem_p(f->dev->sem_read);//wait until there is an available button
...
static int dev_read_ser(FileObject *f, void *buf, size_t len)
{
	...
	USART_ReadBlocking(USART0, buff+f->offset, len);

//	while(cCount < len)
//	{
////		buff[cCount+f->offset] = USART_ReadBlocking(USART2, buff, 1);
//		buff[cCount+f->offset] = (char)getchar();
//		cCount++;
//	}
	f->offset += len;
	sem_v(f->dev->mutex);
